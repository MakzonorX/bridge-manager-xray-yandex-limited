...
echo "[3/10] diagnostics"
curl -sS "${API_BASE}/v1/system/diagnostics" \
  -H "Authorization: Bearer ${API_TOKEN}" | jq .

echo "[4/10] create user"
...
echo "[7/10] get limit-policy (default unlimited)"
curl -sS "${API_BASE}/v1/users/${TEST_USER_ID}/limit-policy" \
  -H "Authorization: Bearer ${API_TOKEN}" | jq .

echo "[8/10] set limit-policy (limited + throttle)"
...
echo "[9/10] get limit-policy (verify)"
curl -sS "${API_BASE}/v1/users/${TEST_USER_ID}/limit-policy" \
  -H "Authorization: Bearer ${API_TOKEN}" | jq .

echo "[10/10] delete user"
